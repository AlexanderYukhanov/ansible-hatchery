...
DOCUMENTATION = '''
---
module: azure_rm_postgresql_configurations_facts
version_added: "2.5"
...
EXAMPLES = '''
  - name: Get instance of Configurations
    azure_rm_postgresql_configurations_facts:
...
class AzureRMConfigurationsFacts(AzureRMModuleBase):
    ...
    def get(self):
        ...
        found = False
        try:
            response = self.mgmt_client.configurations.get(self.resource_group_name,
                                                           self.server_name,
    ...
    def list_by_server(self):
        ...
        found = False
        try:
            response = self.mgmt_client.configurations.list_by_server(self.resource_group_name,
                                                                      self.server_name)
            found = True
            self.log("Response : {0}".format(response))
            self.log("Configurations instance : {0} found".format(response.name))
